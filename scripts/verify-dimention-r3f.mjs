...
try {
  ...
  for (const label of ["큐브 1 · 위 값", "큐브 2 · 왼쪽 아래 값", "큐브 3 · 오른쪽 아래 값"]) await page.getByLabel(label).fill("12");
  await page.getByLabel("반사 유지율 값").fill("0.98");
  await page.getByText("고급 광학 설정", { exact: true }).click();
  ...
  for (const label of ["큐브 1 · 위 값", "큐브 2 · 왼쪽 아래 값", "큐브 3 · 오른쪽 아래 값"]) await page.getByLabel(label).fill("7");
  ...
  if (Object.values(checks).some((value) => !value)) throw new Error(`Dimention R3F verification failed: ${JSON.stringify({ checks, stressLightingFrames, loopLightingFrames, initialContrast, adjustedContrast, mirrorDifference, detailedLightDifference, orbitContrast, pngLength: png?.length, errors })}`);
