...
export interface DimentionR3FState {
  ...
  mirror: {
    ...
    /** Stable solid IDs: upper, lower-left, lower-right at the initial pose. */
    cubeBounces: [number, number, number];
...
const base = (): DimentionR3FState => ({
  ...
  mirror: { enabled: true, intensity: 1, bounces: 7, cubeBounces: [7, 7, 7], recursionScale: .72, reflectivity: .84, absorption: .16, dispersion: .055, edgeIntensity: 1.25, fresnelBoost: 1, backfaceEnergy: .52, depthShift: 1, blur: 1, lightThreshold: .09 },
  ...
  export: { ppi: 300, imageResolution: "artboard", videoFps: 30, videoResolution: "4k", videoWidth: 3840, videoHeight: 2160, videoBitrateMbps: 160, videoSupersampling: 2 },
});
...
export function sanitizeDimentionR3FState(value: unknown): DimentionR3FState {
  ...
  const legacyBounces = Math.round(number(candidate?.mirror?.bounces ?? (candidate?.mirror as unknown as { layers?: number })?.layers, fallback.mirror.bounces, 0, 24));
  ...
  return {
    ...
    mirror: {
      ...
      bounces: legacyBounces,
      cubeBounces: [0, 1, 2].map(index => Math.round(number(candidate?.mirror?.cubeBounces?.[index], legacyBounces, 0, 24))) as [number, number, number],
    ...
    export: {
      ...
      imageResolution: candidate?.export?.imageResolution === "4k" ? "4k" : "artboard",
