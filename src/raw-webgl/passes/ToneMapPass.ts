...
export class ToneMapPass {
  ...
  render(source: Texture2D, bloom: Texture2D, surface: PassSurface, post: Readonly<RawPostState>): void {
    ...
    bindSampler2D(this.gl, this.state, this.program, "uBloom", bloom.handle, 1);
    ...
    uniform1i(this.gl, this.program, "uBloomEnabled", post.bloomEnabled ? 1 : 0);
    uniform1f(this.gl, this.program, "uBloomStrength", post.bloomStrength);
    uniform1f(this.gl, this.program, "uBloomRadius", post.bloomRadius);
    uniform1f(this.gl, this.program, "uBloomThreshold", post.bloomThreshold);
