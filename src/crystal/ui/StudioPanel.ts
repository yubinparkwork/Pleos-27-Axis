...
export function studioPanelTemplate(model: StudioPanelModel): string {
  ...
  const exportMotion = details("모션 시퀀스", `<button class="compact-action" data-action="copy-sequence">PNG 시퀀스 명령 복사</button><code data-output="sequence-command" hidden></code>`, "export-motion");
  const exportPanel = section("이미지 내보내기", `
        ...
        ${exportRegion}
        <div class="export-block"><strong>렌더</strong>${selectRow("유형", `<select data-export-type><option value="still">이미지</option><option value="motion">PNG 시퀀스</option></select>`)}${selectRow("렌더러", `<select data-export-render><option value="path">패스 트레이싱</option><option value="raster">래스터</option></select>`)}<div data-path-settings>${property("scale", "렌더 비율", .4, 1, .05, model.renderScale)}${property("bounces", "반사 횟수", 3, 14, 1, model.bounces)}${property("target-samples", "샘플", 16, 512, 16, model.outputSamples)}</div></div>
        ...
        ${exportMotion}
        <div class="render-progress"><div><span data-output="render-progress-text">준비됨</span><b data-output="render-progress-percent">0%</b></div><i><span data-output="render-progress-bar"></span></i></div><button class="render-primary" data-action="render-export">PNG 내보내기</button><button class="render-cancel" data-action="cancel-render" hidden>취소</button>`, "data-output-section data-panel-section='output'");
  const structurePanel = `<aside class="workspace-panel structure-panel" aria-label="구조 및 컴포지션 패널">
    ...
    <div class="panel-scroll">
      ...
      ${section("카메라", `<label class="toggle-row"><span>시점 잠금</span><input data-control="view-lock" type="checkbox" ${model.viewLocked === false ? "" : "checked"}></label>${property("preview-zoom", "미리보기 확대", .5, 1.8, .05, model.artboard.previewZoom)}<button class="compact-action" data-action="reset">카메라 초기화</button>`, "data-panel-section='camera'")}
      ${section("구도", `${property("artboard-scale", "그래픽 크기", .25, 2, .01, model.artboard.scale)}${property("axis-anchor-y", "세로 위치", 0, 1, .01, model.artboard.axisAnchor.gridY)}${property("axis-anchor-x", "가로 위치", 0, 1, .01, model.artboard.axisAnchor.gridX)}${selectRow("맞춤", `<select data-format="fit"><option value="contain">Contain</option><option value="cover">Cover</option><option value="custom">Custom</option></select>`)}<label class="toggle-row"><span>안전 가이드</span><input data-format="safe-guide" type="checkbox" ${model.artboard.safeGuide ? "checked" : ""}></label>`, "data-panel-section='composition'")}
    </div>
